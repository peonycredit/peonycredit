...
#if defined(NDEBUG)
# error "Peonycredit cannot be compiled without assertions."
#endif
...
static const int64_t nTargetTimespanP1 = 80 * 60;
static const int64_t nTargetTimespanP2 = 60;
static const int64_t nTargetSpacingP1 = 30;
static const int64_t nTargetSpacingP2 = 60;
static const int64_t nInterval = 240;

static const int64_t nAveragingInterval = 240;
static const int64_t nAveragingInterval4 = 10;
static const int64_t nAveragingTargetTimespanP1 = nAveragingInterval * nTargetSpacingP1;
static const int64_t nAveragingTargetTimespanP2 = nAveragingInterval4 * nTargetSpacingP2;

static const int64_t nMaxAdjustDown = 2; // 2% adjustment down
static const int64_t nMaxAdjustUpV1 = 10; // 10% adjustment up
...
*/
    ...
    if (pindexLast->nHeight >= nBlockTimeWarpPreventStart4)
	{
        // find first block in averaging interval
        // Go back by what we want to be nAveragingInterval blocks
        pindexFirst = pindexPrev;
        for (int i = 0; pindexFirst && i < nAveragingInterval4 - 1; i++)
        {
            pindexFirst = pindexFirst->pprev;
            pindexFirst = GetLastBlockIndexForAlgo(pindexFirst, algo);
        }
...
bool static ProcessMessage(CNode* pfrom, string strCommand, CDataStream& vRecv)
{
    ...
    if (strCommand == "version")
    {
        ...
        if (!vRecv.empty()) {
            ...
            if (
                 (pfrom->cleanSubVer == "/Satoshi:1.0.0.4/") ||
				 (pfrom->cleanSubVer == "/Satoshi:2.0.0.1/") ||
				 (pfrom->cleanSubVer == "/Satoshi:2.0.0.2/") ||
				 (pfrom->cleanSubVer == "/Satoshi:3.0.0.1/") ||
                 (pfrom->cleanSubVer == "/Satoshi:1.0.0.5/") ||
                 (pfrom->cleanSubVer == "/Satoshi:1.0.0.6")
               )
            {
                // disconnect from peers older than this proto version
